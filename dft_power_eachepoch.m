function [data_power, freq] = dft_power_eachepoch(Y, srate, cndidx, varargin)
...
if ~OK
    error('dft_power_eachepoch: Non recognized inputs')
end
...
for i=1:Ncnd
    cndi = theCNDs(i);
    data_power(:,:,i) = 2 * nanmean(abs( Ydft(:,:,cndidx==cndi)).^2,3) / (Ns.^2); % Power Spectrum
end
